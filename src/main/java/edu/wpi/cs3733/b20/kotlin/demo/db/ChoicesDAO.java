...
public class ChoicesDAO {
	...
	public Choice getChoice(String uuid) throws Exception{
		Choice choice = null;
		try {
			//look through choices table for choices with a matching UUID
			...
			//look through the alternatives table for alts with a matching UUID
			...
			while(alternativeSet.next()) {
				...
				alternatives.add(alternativeSet.getInt("altID"), alt); //add alt as position "altID"
			}
